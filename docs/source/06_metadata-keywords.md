...
```{eval-rst}
.. tabularcolumns:: |\Y{0.15}|\Y{0.55}|\Y{0.30}|
```
...
### Associated information: More on Class Keywords
...
GENERAL Subclass Keywords &mdash; `absolute`, `average`, `center`, `component`, `derived`, `instrument`, `interval`, `maximum`, `measured`, `minimum`, `mean`, `offset`, `spacecraft`, `vector`, `total`

COORDINATE SYSTEM Subclass Keywords &mdash; `cartesian`, `geographic`, `geomagnetic`, `GCI`, `GSM`, `GSE`, `HDZ`, `HGI`, `NEV`, `polar`   

SOURCE Subclass Keywords &mdash; `electron`, `ion`, `proton`, `helium`, `nitrogen`, `oxygen`, `Z>3`, `neutral` , `particle`, `dust`

WAVELENGTH Subclass Keywords &mdash; `radio`, `IR`, `visible`, `UV`, `x-ray`, `gamma` `ray`

### Examples of Subclass Keywords
