...
- [CDF](https://cdf.gsfc.nasa.gov/) in the rest of Heliophysics with ISTP metadata. Also, CDF-A, defined as CDF files with ISTP and [SPASE](https://spase-group.org/) metadata, but without compression or sparse variables, is one of standard formats (in addition to PDS-3, PDS-4, JPEG) in Planetary Data System ([PDS](https://pds.nasa.gov/)).
...
Variable names, and global and variable attribute names are case-sensitive in CDF and netCDF formats. The ISTP attribute names use a mixture of case in their definitions, which should be preserved.  However, there are datasets where some variations were used, so generalized software should match in a case-insensitive way when acting on the attributes.
...
- Built-in support for data compression (gzip, RLE, Huffman) and automatic data uncompression on file and individual variable levels, and checksums
