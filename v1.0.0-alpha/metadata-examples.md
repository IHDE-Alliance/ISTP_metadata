...
Â· Density and Temperature (**data**) in this example are scalars; in CDF they are associated with zero dimension and no size.

Â· Plasma velocity, electric and magnetic fields (**data**) are vectors, i.e., three
orthogonal components in some coordinate system, stored in one-dimension of size 3.

Â· Particle flux, for this example, has values at eight energy channels. Flux (**data**) and Energy (**support_data**) are stored as one-dimensional variables of size 8.

Â· The image array (**data**) maps into a two-dimensional variable with sizes 256 and 256. Latitude and longitude (**support_data**) are one-dimensional variables of size 256, providing the necessary coordinate indices for the image array.
...
! -------- ---- -------- ---- ----- -------- ---------
 ...
 "VAR_NOTES" CDF_CHAR { "Assuming no helium (0.3 - several " - "hundred) if the density is less than " - "0.3/cc the higher moments (VEL,TEMP) " - "shall not be used because of the poor " - "counting statistics." }
